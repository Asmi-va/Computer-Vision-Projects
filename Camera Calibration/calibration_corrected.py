...
input_params = {
    "input": "D:\\Computer-Vision-Projects-main\\Camera Calibration\\video.mp4",  # Path to input video file
    ...
    "output": "D:\\Computer-Vision-Projects-main\\Camera Calibration\\yaml\\params.yaml",  # Path to output YAML file
...
def main():
    ...
    if not vdo.isOpened():
        raise IOError(f"Cannot open video {input_params['input']}")

    while True:
        i += 1
        success, img = vdo.read()
        if not success:
            print("Frame read unsuccessful or no more frames available.")
            break
        ...
        if input_params["flip"] != 0:
            img = cv2.flip(img, input_params["flip"])
